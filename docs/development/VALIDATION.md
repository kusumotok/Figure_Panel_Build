...
## 2026-09-11: Undo/Redo と PDF の方針（S11 の一部）

147 tests, 0 failures, 0 errors。GUI 検証 23 項目すべて通過。

### PDF の判断（計画で保留していた点）

**PDF は PPTX 経由に留める**（ユーザー判断）。PDFBox 等を追加するとベクタ + フォント埋め込みの PDF を直接書けるが、JAR が数 MB 増える。PowerPoint の「PDF として保存」がフォント埋め込みとベクタ保持を満たすため、依存を増やす価値が見合わないと判断した。bleed・trim mark が必要になった時点で再検討する。

### Undo / Redo

削除やマージが取り消せない状態だったので追加した。既存 `FigureHistory` と同じくドキュメント全体の JSON スナップショット方式（ドキュメントは画素を含まないので軽い）。100 件上限、同種の連続編集は 600 ms 以内なら 1 ステップにまとめる。`Ctrl+Z` / `Ctrl+Y`。

**記録位置を relayout ではなく変更点に置いた。** 当初 relayout に 1 箇所だけフックしたが、描画側の副作用でも履歴が動きうるため、canvas / inspector / 右クリックメニュー / 削除 / 取り込み / スタイル適用の各所で明示的に記録する形にした。プロジェクトを開くときは履歴をクリアする（別プロジェクトへ undo できてしまわないように）。

### 検証中の誤診について

GUI 検証で redo だけ失敗し、一度「既知の不具合」として記録しかけたが、**原因は検証コードが別の変数（mm 単位の幅）と比較していたテストのバグ**だった。製品側は正常。誤った注記は削除済み。

## 2026-09-11 (2): 不均等レイアウトの実データ検証と列計測の修正

147 tests, 0 failures, 0 errors。

Fiji は `C:\Users\user\Apps\Fiji`（書き込み可能）に移動。MCP もそちらを起動し、`Poster / Layout Builder` の登録と起動を実機で確認した。

### 見つけて直した不具合

実データで「1 枚だけ大きい」構成を組んだところ、ページが 180 x 233 mm に膨張し、大パネルが 202 mm（ページ幅超過）になった。**Fill 幅のキャプションを計測する際に幅が渡らず、テキスト計測器が折り返し無しの全長（約 411 mm）を返し、それを AUTO 列が採用していた。**

対処は 2 つ:

1. 列の計測で、Fill 以外の子には親の内寸を上限として渡す（自分の列が fraction のコンテナが固有印刷サイズにフォールバックしないように）。Fill の子は列幅への要求を持たないので 0 のまま
2. **AUTO 列の base を親の内寸で頭打ちにする。** AUTO 列が親より広くて意味のある場面は無い。Fixed とアスペクト由来のトラックは据え置きなので、本当の過剰制約は引き続き警告される

修正後: 180 x 112 mm、大パネル 81 x 81 mm、警告ゼロ、全 5 枚が 300 dpi 以上。

### 実データでの成果物

`UnevenFigureValidation`（`artifacts/uneven/`）: lif から 6 視野を 2 series ずつ合成し、代表 1 枚を 2 行ぶんに span、残りを狭い 2 列に、下にキャプション。旧 Figure モードでは表現できない構成。

### 次に直すべき点

**パネルの大きさが違うとスケールバーの見た目が揃わない。** バーの太さと文字サイズはソース px に対する比率で決まるため、大きく表示したパネルではバーも大きく描かれる。均一グリッドでは問題にならなかったが、不均等レイアウトでは目立つ。物理サイズ（mm）基準で指定できるようにするのが筋。

## 2026-09-11 (3): 保存済みプロジェクトをセルに添付する

152 tests, 0 failures, 0 errors。

「PPTX のセルに PPTX をアタッチできるか」への回答として実装した。**本ツールが保存した PPTX のみ**が対象。任意の PowerPoint ファイルは、こちらでレイアウト・描画する手段が無いため添付できない（サムネイルを貼るのは添付とは呼べない）。

### リンクと埋め込みの両方

既定は**リンク**。セルは別 PPTX の project part を参照し、開くたびに読み直すので、図版を直せばそれを使うポスター側も追従する。右クリックの `Embed this figure (cut the link)` でその場に展開し、リンクを切る。ページレイアウトソフトの配置画像と同じ発想。

- 添付図版は**再フローしない**。自分のページサイズでレイアウトしたうえで、セルに収まるよう一様に縮小する。図版はどこに置いても同じ見た目であるべきだから
- セルのアスペクトは添付ページの物理サイズから決まる
- 埋め込み時はノード ID・アセット・スタイル・トークンに接頭辞を付けて取り込むので、同じ図版を 2 つ並べても衝突しない
- PPTX 書き出しでは、添付図版の shape を**同じスライドに座標変換して展開**する。図版のテキストは PowerPoint で編集可能なまま（画像に潰れない）

### 検証中に見つけて直した不具合

1. **循環参照のガードが効かず StackOverflow。** `LinkedProjects` が内部で既定の `LayoutEngine` を作っており、そちらが別の `LinkedProjects` を持つためガードを共有できていなかった。自分自身を参照する PPTX で再現。共有するよう修正し、外側のファイル自体は開けて内側の自己参照だけがプレースホルダになることを確認
2. **入れ子側のアセットが引けず「Unknown asset」。** ホストの `AssetLibrary` は添付ドキュメントのアセットを知らない。`Attached` ごとに自前のライブラリを持たせた

### 実データでの成果物

`AttachedPosterValidation`（`artifacts/attached/`）: A1 ポスターの 2 セルに、先の不均等図版（それ自体が入れ子グリッド + span + キャプション）を添付。片方はリンクのまま、もう片方は埋め込み。

### 制限

- 任意の PowerPoint ファイルは添付できない
- 添付元が変わったことは検出できるが（fingerprint）、現状は報告のみで再読込は開き直したとき
